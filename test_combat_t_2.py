...
# weapon = weapon("rarity", "name","type","dmg","Atkspeed","range","hits")
weapon_1 = weapon(BLACK("Common"), "Dagger", Blade, 15, 20, 1, 1)
weapon_2 = weapon(GREEN("Uncommon"), "Rapier", Blade, 45, 10, 20, 1)
weapon_3 = weapon(BLUE("Rare"), "Zweihander", Blade, 60, 8, 25, 1)
weapon_4 = weapon(BLUE("Rare"), "Shorre", Gun, 50, 7, 30, 5)
weapon_5 = weapon(MAGENTA("Epic"), "Ak 5", Gun, 7, 25, 100, 20)
weapon_6 = weapon(YELLOW("Legendary"), "Golden Scar", Gun, 10, 30, 150, 15)
weapon_7 = weapon(RED("Mythic"), "Musket", Gun, 2000, 1, 200, 1)

weapon_4.Hits_W = r.randint(0, 5)
weapon_5.Hits_W = r.randint(0, 20)
weapon_6.Hits_W = r.randint(0, 15)
...
# Helper function to create a fresh copy of a random entity for each encounter
def spawn_random_entity():
    """Spawn a fresh random entity from the entity list."""
    template = r.choice(entity_n)
    # Create a new instance with the same stats
    return entity(template.Name_E, template.Element_E, template.HP_E, template.Weak_E, template.STR_E, template.AtkSpeed_E)
            ...
            if door == "left" or door == "middle" or door == "right":
                ...
                # Här skapas en strängrepresentation av fienden som valts och vad som händer vid en encounter
                    def Entity_in_room_function(monster):
                        ...
                        if action == "a":
                            # Generate a NEW monster when player chooses to attack
                            monster = spawn_random_entity()
                            if weapon_inventory:
                                print("Your weapons:")
                            ...
                            if chosen_weapon is None:
                                print(RED("\nYou don't have that weapon. Using your first weapon instead.\n"))
                                chosen_weapon = weapon_inventory[0]

                            # compare attack speed and resolve
                            if chosen_weapon.AtkSpeed_W > monster.AtkSpeed_E:
                                print(f"\nYou have chosen to attack with the {chosen_weapon.Name_W}.\n")
                            ...
                            if escape == 1:
                                ...
                                # Reuse the SAME monster (pass monster, not a new one)
                                Entity_in_room_function(monster)
                            print(f"You have now entered {room_encounter} rooms.\n")
                    Entity_in_room_function(monster=spawn_random_entity())
                        

                    
            # Här hanteras loot rooms, trap rooms och tomma rum
                else:
                    loot_room = r.randint(1,1)
                    if loot_room == 1:
                        print(YELLOW("\nYou have found a loot room!\n"))
                        weapon_chest = r.randint(1,1)
